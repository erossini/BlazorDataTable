...
namespace PSC.Blazor.Components.DataTable.Code.Enumerations
{
	...
	public abstract class Enumeration : IComparable
	{
		...
		public static T FromDisplayName<T>(string displayName) where T : Enumeration =>
			Enumeration.Parse<T, string>(displayName, "display name", item => item.Name == displayName);
		...
		private static T Parse<T, K>(K value, string description, Func<T, bool> predicate) where T : Enumeration =>
			Enumeration.GetAll<T>().FirstOrDefault<T>(predicate) ?? throw new InvalidOperationException(string.Format("'{0}' is not a valid {1} in {2}", value, description, typeof(T)));
